#CLIP — Eyecontact(with margin threshold + face-crop + denser sampling)
...
# Decision margin for confidence
MARGIN_THRESHOLD = 0.01

# Face crop settings
FACE_CROP = True
FACE_PADDING = 0.20  # expand detected box by 20%

# Zero-shot prompts for eye contact
# Camera is behind the interviewer; interviewee looking toward interviewer ≈ looking toward camera.
TEXT_PROMPTS = [
    "the interviewee making eye contact with the interviewer or looking towards",
...
def analyze_video(video_path: str, spacing_sec: float, save_frames: bool):
    ...
    frame_save_dir = os.path.join(OUTPUT_DIR, "sampled_frames_clip_eye")
    if save_frames:
        os.makedirs(frame_save_dir, exist_ok=True)

    results = []
    for fi in tqdm(idxs, desc="Evaluating frames (CLIP, eye contact)"):
        ...
        if save_frames:
            Image.fromarray(rgb_img).save(os.path.join(frame_save_dir, f"frame_{fi:06d}.jpg"), quality=95)
        ...
        with torch.no_grad():
            ...
            sim_eye = sim[0, 0].item()  # making eye contact
            sim_not = sim[0, 1].item()  # not making eye contact
...
if __name__ == "__main__":
    ...
    yes_count = (df_sorted["eye_contact"] == "Yes").sum()
    no_count = (df_sorted["eye_contact"] == "No").sum()
    uncertain_count = (df_sorted["eye_contact"] == "Uncertain").sum()
    decided = yes_count + no_count

    majority_label = "Tie" if decided == 0 or yes_count == no_count else ("Yes" if yes_count > no_count else "No")
    yes_pct = (yes_count / decided * 100.0) if decided > 0 else 0.0

    base = os.path.splitext(os.path.basename(VIDEO_PATH))[0]
    csv_path = os.path.join(OUTPUT_DIR, f"{base}_eye_contact_clip_dense_face_threshold.csv")
    df_sorted.to_csv(csv_path, index=False)

    duration_sec = total_frames / fps if fps > 0 else float("nan")
    print("\n--- Summary (CLIP • Eye Contact) ---")
    ...
    print(f"Per-frame eye contact: Yes={yes_count}, No={no_count}, Uncertain={uncertain_count}, Yes% (of decided)={yes_pct:.1f}%")
