#SigLIP version (with margin threshold + face-crop + denser sampling)
...
# Face crop settings
FACE_CROP = True
FACE_PADDING = 0.20  # expand detected box by 20% (clamped to image)

# Zero-shot prompts
TEXT_PROMPTS = ["a person smiling", "a person not smiling"]

# -----------------------
# Load SigLIP (drop-in for CLIP)
# -----------------------
...
def analyze_video(video_path: str, spacing_sec: float, save_frames: bool):
    ...
    frame_save_dir = os.path.join(OUTPUT_DIR, "sampled_frames_siglip")
    if save_frames:
        os.makedirs(frame_save_dir, exist_ok=True)

    results = []
    for fi in tqdm(idxs, desc="Evaluating frames (SigLIP)"):
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"{base}_smile_majority_siglip_dense_face_threshold.csv")
