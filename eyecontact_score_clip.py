#CLIP - score eye contact in images 2-7
...
SCORE_MIN = 2.0
SCORE_MAX = 7.0
MARGIN_MIN = -0.05   # strong "not eye contact"
MARGIN_MAX =  0.05   # strong "eye contact"
...
def margin_to_score(margin: float) -> float:
    # Linear map from [MARGIN_MIN, MARGIN_MAX] -> [SCORE_MIN, SCORE_MAX], clamped
...
def analyze_video(video_path: str, spacing_sec: float, save_frames: bool):
    ...
    for fi in tqdm(idxs, desc="Evaluating frames (CLIP, eye contact)"):
        ...
        with torch.no_grad():
            ...
            # Label (still provided for transparency)
...
if __name__ == "__main__":
    ...
    # Average mark (prefer frames where the face was detected)
    ...
    # Also report label counts (informational)
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"{base}_eye_contact_clip_marks.csv")
    df_sorted.to_csv(csv_path, index=False)

    duration_sec = total_frames / fps if fps > 0 else float("nan")
    print("\n--- Summary (CLIP • Eye Contact • Marks 2–7) ---")
